...
class TestAssetClassifier(unittest.TestCase):
    def test_infer_cde(self):
        ...
        self.assertEqual(ac.classify_asset("b2p-tokenize.dev2.bee2pay.com", "/"), "cde")
        self.assertEqual(ac.classify_asset("3ds.bee2pay.com", "/"), "cde")

    def test_infer_funds(self):
        import asset_classifier as ac
        self.assertEqual(ac.classify_asset("b2p-gateway-cielo.hml2.bee2pay.com", "/"), "funds")
        self.assertEqual(ac.classify_asset("checkout.bee2pay.com", "/"), "funds")

    def test_infer_internal_and_public(self):
        import asset_classifier as ac
        self.assertEqual(ac.classify_asset("b2p-hml-kibana-v2.hml2.bee2pay.com", "/"), "internal")
        self.assertEqual(ac.classify_asset("www.bee2pay.com", "/"), "public")
    ...
    def test_override_wins(self):
        import asset_classifier as ac
        ov = [{"match": "checkout.bee2pay.com", "data_class": "cde"}]
        self.assertEqual(ac.classify_asset("checkout.bee2pay.com", "/", ov), "cde")
...
(Bee2pay-aware) com override opcional por arquivo. Override > inferência > default.
"""
import fnmatch
import re
...
_PATTERNS = [
    ("cde",      r"card|pan|tokenize|vcn|3ds|acs|maxipago|braspag"),
    ...
    ("pii",      r"auth|token|login|onetime|sso|saml|oauth|keycloak|kc\.|back-token|"
                 r"fraudguard|buyer|reservation|hotel|chargeback|receivable|"
...
class TestEnrichFindings(unittest.TestCase):
    def test_enrich_adds_criticality_fields(self):
        ...
        findings = [
            {"name": "IDOR", "severity": "medium", "vuln_class": "idor_read_pii",
             "url": "https://b2p-tokenize.dev2.bee2pay.com/api/x", "confirmed": True},
        ]
    ...
    def test_enrich_infers_host_from_url(self):
        import criticality as cr
        findings = [{"name": "x", "severity": "high", "url": "https://www.bee2pay.com/p"}]
        out = cr.enrich_findings(findings, overrides=None)
